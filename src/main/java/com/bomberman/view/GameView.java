...
public class GameView {
    ...
    private void drawPlayers(java.util.List<Player> players) {
        for (Player player : players) {
            ...
            gc.fillOval(x + 3, y + 3, GameConstants.CELL_SIZE - 6, GameConstants.CELL_SIZE - 6);

            // Bordure
            gc.setStroke(Color.BLACK);
            gc.setLineWidth(2);
            gc.strokeOval(x + 3, y + 3, GameConstants.CELL_SIZE - 6, GameConstants.CELL_SIZE - 6);

            // Numéro du joueur
            gc.setFill(Color.WHITE);
            gc.setFont(javafx.scene.text.Font.font(14));
            gc.fillText(String.valueOf(player.getId() + 1),
                    x + GameConstants.CELL_SIZE/2 - 3,
                    y + GameConstants.CELL_SIZE/2 + 4);
        }
    }
}